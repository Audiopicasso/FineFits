...
export function ColorEyedropper({ imageUrl, onColorSelect, trigger }: ColorEyedropperProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!ctx) {
        setError('Canvas-Kontext konnte nicht geladen werden');
        setIsLoading(false);
        return;
      }
      ...
      fetch(imageUrl, { credentials: 'include' })
        ...
        .then(blob => {
          ...
          img.onerror = () => {
            setError('Bild aus Blob konnte nicht geladen werden');
            setIsLoading(false);
          };
          img.src = blobUrl;
        })
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Farbe aus Bild wählen
            </DialogTitle>
          </DialogHeader>

          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Klicke irgendwo auf das Bild, um eine Farbe zu entnehmen
            </p>
              ...
              <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                <div className="flex items-center gap-3">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    />
                    <span className="text-xs text-muted-foreground">Gewählt</span>
                  </div>
